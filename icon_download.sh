...
shall_i_delete() {
    ...
    if [[ -d "${DIR}" ]] && [[ -n "${delete}" ]] && [[ "${delete}" == "yes" ]]; then
...
exit_time() {
    if [[ -n "${sleeptimehuman}" ]]; then
        if [[ "${sleeptimehuman}" == "30 seconds" ]]; then
            sleeptime=30
		elif [[ "${sleeptimehuman}" == "1 minute" ]]; then
            sleeptime=60
		elif [[ "${sleeptimehuman}" == "2 minutes" ]]; then
            sleeptime=120
		elif [[ "${sleeptimehuman}" == "5 minutes" ]]; then
            sleeptime=300
		elif [[ "${sleeptimehuman}" == "10 minutes" ]]; then
            sleeptime=600
		else
			echo "Invalid human sleep time set"
        fi
    fi
    
    if [[ -n "${sleeptime}" ]]; then
        echo "I will now exit in ${sleeptimehuman} to allow time for icons to sync"
        sleep "${sleeptime}"
    else
        echo "I will now exit immediately"
        sleeptime=0
    fi
}
